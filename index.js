...
const coinName = "BraveMoon";
const coinSymbol = "Bmoon";
...
const uploadImage = async () => {
  ...
  const imageFile = fs.readFileSync("./image1.jpg");
...
const uploadMetadata = async (imageUri) => {
  ...
  const metadata = {
    name: coinName,
    symbol: coinSymbol,
    description: "BraveMoon is the Solana meme coin built for winners. Lightning-fast trades, tiny fees, and a community that’s ready to moon. Memes drive us, trust binds us—join the pack and ride the ultimate crypto wave to riches!",
    image: imageUri,
    "twitter": "https://x.com/bravemooncoin",
  };
...
const mintToken = async (imageUri, metadataUri) => {
  ...
  // Mint the token.
  let numDecimals = 8;
  const totalSupply = 1000000000000 * Math.pow(10, numDecimals);
